...
app.get('/api/summary', requireAdmin, async (req, res) => {
  try {
    ...
    // Obtener max_guests de settings
    let maxGuests = 100;
    try {
    ...
    // Obtener conteo de invitados por prioridad
    ...
    res.json({
      totalGroups: groupCount.totalGroups || 0,
      totalGuests: stats.totalGuests || 0,
      confirmedGuests: stats.confirmedGuests || 0,
      declinedGuests: stats.declinedGuests || 0,
      pendingGuests: stats.pendingGuests || 0,
      confirmedChildMenus: stats.confirmedChildMenus || 0,
      confirmedAdults: stats.confirmedAdults || 0,
      maxGuests: maxGuests,
...
/**
 * POST /api/groups
 * Crea un grupo y opcionalmente añade sus invitados iniciales en una transacción.
 */
    ...
    try {
      ...
      if (Array.isArray(guests)) {
        for (const guest of guests) {
          ...
          if (!guestName) continue; // Salta invitados sin nombre ni tipo válido
 ...
 * Elimina un grupo y todos sus invitados asociados por cascada.
 */
...
/**
 * POST /api/guests
 * Agrega un nuevo invitado a un grupo existente.
 */
app.post('/api/guests', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // Verificar que el grupo realmente exista
...
/**
 * PUT /api/guests/:id
 * Actualiza los datos de un invitado (confirmación, menú de niños, etc.).
 */
 ...
 * Elimina un invitado individual.
 */
...
/**
 * POST /api/categories
 * Registra una nueva categoría de grupo.
 */
app.post('/api/categories', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // Verificar si ya existe para evitar errores de llave única
 ...
 * Obtiene todos los conceptos de apoyo.
 */
 ...
 * Crea un nuevo concepto de apoyo.
 */
...
/**
 * GET /api/godparents
 * Obtiene todos los padrinos con sus conceptos.
 */
...
app.post('/api/godparents', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // Verificar que el concepto exista si se proporciona
    if (concept_id) {
...
// Carpeta de archivos estáticos compilados del Frontend
...
app.listen(PORT, async () => {
  console.log(`Servidor de bodas escuchando en el puerto ${PORT}`);
  try {
    await getDb();
    console.log('Base de datos inicializada y migraciones ejecutadas con éxito.');
  } catch (err) {
    console.error('Error crítico al inicializar la base de datos en el arranque:', err);
    process.exit(1);
  }
});
